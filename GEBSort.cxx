...
int main (int argc, char **argv) {
  // ROOT TTree functionality added by Chris Sullivan (MSU/NSCL)
  // for use during July 2015 CAGRA+GR commissioning run
...
{
  TTree *rawtree = new TTree("rawtree","rawtree"); // CAGRA+GR raw tree output file
  DGSEvent *dgsevent = new DGSEvent;
  rawtree->Branch("dgsevent",&dgsevent); // 120 bytes - 7/5/15
  dgsevent->Clear();
  //rawtree->BranchRef();
...
#include "UserPreCond.h"
          ...
          bin_dgs (&GEB_event,dgsevent,rawtree);
	  rawtree->Fill();
	  dgsevent->Clear();	    
	  if (evtcounter % 200000 == 0) std::cout << "Processed events: " << evtcounter << std::endl;
...
#include "UserEv.h"
  ...
  // Save the CAGRA+GR rawtree
  std::cout << "bin_dgs was called " << evtcounter << " times!" << std::endl;
